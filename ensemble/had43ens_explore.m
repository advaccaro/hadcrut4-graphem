function [explored] = had43ens_explore(infile);
%loads infilled member of HadCRUT4.3 ensemble and prepares it for comparison (calculate GMMT, NINO3.4, etc.)
% only works with infilled members of HadCRUT4.3 (needs correctly formatted input files)

E = load(infile)


% fix grids
lon = double(E.H43ens.lon); lon(lon<0) = lon(lon<0) + 360;
lat = double(E.H43ens.lat);loc = E.loc;
%t=double(E.H43ens.tfrac); %fractional time axis
t = E.H43ens.tser;
