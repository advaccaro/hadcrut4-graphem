function [X, M, C, B, S, kavlr, kmisr, iptrn, peff, D, Cf] = graphem_JW(X, options)
...
%    which contain
%
%       B:     Matrix of regression coefficents of missing values on
...
while (it < maxit && rdXmis > stagtol)
        ...
        if pm > 0
            ...
            switch regress
                case 'mridge'
                    ...
                    [B, S, ~, peff] = mridge(C(kavlr{j},kavlr{j}), ...
                        C(kmisr{j},kmisr{j}), ...
                ...
                case 'iridge'
                    ...
                    [B, S, ~, peff] = iridge(C(kavlr{j},kavlr{j}), ...
                        C(kmisr{j},kmisr{j}), ...
                        C(kavlr{j},kmisr{j}), n-1, optreg);
            ...
            % S       = S .* repmat(D(kmisr{j})', pm, 1) .* repmat(D(kmisr{j}), 1, pm);
    ...
    if useggm
        ...
        [C,sp_level] = Sigma_G_JW(cov(X)+CovRes/dofC,opt_ggm);
	 else
        C = (X'*X + CovRes)/dofC;
    end
...
%     pm     = length(kmisr{j}); % number of missing values in this pattern
%     if pm > 0
%         B = diag(1./D(kavlr{j})) * B * diag(D(kmisr{j}));
%     else
%         B = NaN;
%     end
% end
...
% scaling
%
% Xc(prows{j}, kmisr{j}) = Xc(prows{j}, kavlr{j}) * B
